...
if [ "${BUILD_MODE}" = sap ]; then
  curl -L -o /root/upper-constraints.txt https://raw.githubusercontent.com/sapcc/requirements/stable/stein-m3/upper-constraints.txt
else
  curl -L -o /root/upper-constraints.txt https://raw.githubusercontent.com/sapcc/requirements/stable/stein/upper-constraints.txt
fi
...
if [ "${BUILD_MODE}" = sap ]; then
  ...
  pip_install --no-compile -c /root/upper-constraints.txt \
    ...
    git+https://github.com/sapcc/openstack-rate-limit-middleware.git@1.0.5
